...
import { ViewData, LayerCategory, layerCategories } from './types'
...
interface DrawingControlsExtendedProps {
  currentViewData: ViewData | null
  currentLayer: string
...
const DrawingControlsExtended: React.FC<DrawingControlsExtendedProps> = ({
  currentViewData,
  currentLayer,
  newNameOfLayer,
  //activePolygonId,
  ...
  const layers = currentViewData?.layers || {}
  const image = currentViewData?.image
  ...
  if (!image) {
    return (
      <div className="drawEditor__modernUI__layersSection__empty">
        ...
        <p>Není vybrán žádný obrázek pro aktuální pohled</p>
      </div>
    )
  }

  let filteredLayerCategories = layerCategories
  if (onlyLayerChoice) {
    filteredLayerCategories = filteredLayerCategories.filter((item) => item.id === onlyLayerChoice)
  }
  return (
      ...
      <div className="drawEditor__modernUI__header">
        <div>
          ...
          <p className="drawEditor__modernUI__header__subtitle">
            Spravujte vrstvy a kreslete polygony - {currentViewData?.viewName}
          </p>
        </div>
      </div>
      ...
      <div className="drawEditor__modernUI__layersSection">
        ...
        <div className="drawEditor__modernUI__categoryFilter">
          ...
          <div className="drawEditor__modernUI__categoryFilter__buttons">
            ...
            {filteredLayerCategories.map((category) => {
                ...
                >
                  ...
                  {layerData?.category && (
                    <span className="drawEditor__modernUI__tabs__tab__category">
                      {
                        filteredLayerCategories.find((cat) => cat.value === layerData.category)
                          ?.label
                      }
                    </span>
                  )}
...
export default DrawingControlsExtended
